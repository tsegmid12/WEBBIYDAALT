...
import { questionBank, questionTypes, categories, levels, users } from '../data/mockData';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const examData = {
        ...
        start_on: new Date(formData.start_date).toISOString(),
        close_on: new Date(formData.close_date).toISOString(),
        duration: parseInt(formData.duration),
        max_attempt: parseInt(formData.max_attempt),
        ...
        grade_point: parseInt(formData.course_grade_contribution || 20),
      };

      console.log('Creating exam via API:', examData);
      console.log('Course ID:', course_id);

      // Try to create exam via API
      try {
        const newExam = await api.courses.createCourseExam(parseInt(course_id), examData);
        console.log('✅ Exam created via API:', newExam);

        // Try to add questions if API created successfully
        if (examQuestions.length > 0) {
          console.log(`Attempting to add ${examQuestions.length} questions...`);
          for (const question of examQuestions) {
            try {
              await api.exams.createExamQuestion(newExam.id, {
                question_id: question.question_id,
                point: question.point,
                order: question.order,
              });
            } catch (qErr) {
              console.warn('Failed to add question:', qErr);
            }
          }
        }

        alert(`Шалгалт амжилттай үүсгэгдлээ!\n\nШалгалт: ${newExam.name}\nАсуултын тоо: ${examQuestions.length}\nНийт оноо: ${totalPoints}`);
        navigate(`/team6/courses/${course_id}/exams`);
        return; // Exit if API creation succeeded

      } catch (apiError) {
        console.error('❌ API creation failed:', apiError.message);
        ...
        const maxId = existingExams.length > 0
          ? Math.max(...existingExams.map(e => e.id), 1000)
          : 1000;
        ...
        const newExam = {
          ...
          grade_point: parseInt(formData.course_grade_contribution || 20),
          course_grade_contribution: parseInt(formData.course_grade_contribution || 20),
          eligible_student_ids: eligibleStudentIds,
          created_at: new Date().toISOString(),
        };

        const maxQuestionId = existingExamQuestions.length > 0
        ...
        localStorage.setItem('all_exams', JSON.stringify([...existingExams, newExam]));
        localStorage.setItem('all_exam_questions', JSON.stringify([...existingExamQuestions, ...newExamQuestions]));

        console.log('✅ Exam saved to localStorage:', newExam);
      ...
      alert(`Шалгалт үүсгэхэд алдаа гарлаа: ${error.message || 'Unknown error'}\n\nДахин оролдоно уу.`);
    }
  };
        ...
        <div className='bg-white rounded-lg shadow p-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-6'>
            <button
              ...
              className={`p-4 border-2 rounded-lg text-left transition-all ${creationMethod === 'rules'
            ...
            <button
              ...
              className={`p-4 border-2 rounded-lg text-left transition-all ${creationMethod === 'specific'
                ? 'border-blue-600 bg-blue-50'
                : 'border-gray-200 hover:border-gray-300'
                }`}>
              <div className='flex items-center gap-3 mb-2'>
