...
const ExamStart = () => {
  ...
  const localStorageSubmissions = JSON.parse(localStorage.getItem('studentSubmissions') || '[]');
  ...
  console.log('Student attempts check:', {
    examId: exam_id,
                ...
                <div className='flex items-start gap-3'>
                  ...
                  <div className='flex-1'>
                    ...
                    {studentAttempts.map((attempt, idx) => (
                      <div key={attempt.id || idx} className='text-xs text-blue-700 mt-1'>
                        {idx + 1}-р оролдлого
                        {attempt.grade_point != null ? `: ${attempt.grade_point.toFixed(1)}%` : ': Үнэлгээ хүлээгдэж байна'}
                        {' '}({new Date(attempt.submit_time).toLocaleDateString('mn-MN')})
                      </div>
                    ))}
                  </div>
                </div>
