    ...
    const fetchExamData = async () => {
      try {
        ...
        try {
          ...
          console.log('Failed to fetch questions:', qErr);
          setQuestions([]);
        }
        ...
        try {
          ...
          console.log('Failed to fetch exam users from API:', uErr);
          setExamUsers([]);
        }

        // Also load submissions from localStorage
        try {
          const localSubmissions = JSON.parse(localStorage.getItem('studentSubmissions') || '[]');
          const examLocalSubmissions = localSubmissions.filter(
            s => s.exam_id === parseInt(exam_id)
          );

          // Merge with API submissions (if any)
          // Filter out duplicates if an API submission has the same ID as a local one
          const mergedSubmissions = [...apiSubmissions];
          examLocalSubmissions.forEach(localSub => {
            if (!mergedSubmissions.some(apiSub => apiSub.id === localSub.id)) {
          ...
          console.log('Loaded submissions:', {
            fromAPI: apiSubmissions.length,
            fromLocalStorage: examLocalSubmissions.length,
    ...
    <div className='space-y-6'>
      ...
      <div className='bg-white rounded-lg shadow p-4'>
        <div className='flex items-center gap-3'>
          <span className={`px-4 py-2 rounded-full text-sm font-medium ${isUpcoming
