...
import { Clock, CheckCircle, AlertTriangle, ChevronLeft, ChevronRight, Image as ImageIcon, GripVertical, Save } from 'lucide-react';
...
const ExamTake = () => {
  ...
  const loadSavedAnswers = () => {
    const saved = localStorage.getItem(`exam_${exam_id}_student_${student_id}_answers`);
    return saved ? JSON.parse(saved) : {};
  };

  const loadSavedTime = () => {
    const saved = localStorage.getItem(`exam_${exam_id}_student_${student_id}_time`);
    if (saved) {
  ...
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(() => {
    const saved = localStorage.getItem(`exam_${exam_id}_student_${student_id}_current`);
  ...
  const [startTime] = useState(() => {
    ...
    const now = new Date();
    localStorage.setItem(`exam_${exam_id}_student_${student_id}_time`, JSON.stringify({
  ...
  const [orderingAnswers, setOrderingAnswers] = useState(() => {
    const saved = localStorage.getItem(`exam_${exam_id}_student_${student_id}_ordering`);
    return saved ? JSON.parse(saved) : {};
  });
  const [questionsPerPage, setQuestionsPerPage] = useState(() => {
    const saved = localStorage.getItem(`exam_${exam_id}_student_${student_id}_perPage`);
    return saved || 1;
  });
  ...
  const autoSave = useCallback(() => {
    setIsSaving(true);
    localStorage.setItem(`exam_${exam_id}_student_${student_id}_answers`, JSON.stringify(answers));
    localStorage.setItem(`exam_${exam_id}_student_${student_id}_ordering`, JSON.stringify(orderingAnswers));
    localStorage.setItem(`exam_${exam_id}_student_${student_id}_current`, currentQuestionIndex.toString());
    localStorage.setItem(`exam_${exam_id}_student_${student_id}_perPage`, questionsPerPage.toString());
    setLastSaveTime(new Date());
    setTimeout(() => setIsSaving(false), 500);
  }, [answers, orderingAnswers, currentQuestionIndex, questionsPerPage, exam_id, student_id]);
  ...
  const handleSubmit = () => {
    ...
    console.log('✅ Exam submitted successfully:', submission);
    console.log('Completion time:', completionTime, 'minutes');
    console.log('Total submissions for this exam:', updatedSubmissions.filter(s => s.exam_id === parseInt(exam_id)).length);
  ...
  const handleAnswerChange = (questionId, answer) => {
    setAnswers(prev => {
      ...
      setTimeout(() => {
        localStorage.setItem(`exam_${exam_id}_student_${student_id}_answers`, JSON.stringify(newAnswers));
      }, 0);
      return newAnswers;
    });
  };
  ...
  useEffect(() => {
    ...
    questions.forEach(q => {
      if (q.question?.type === 'ordering') {
        ...
        newOrderingAnswers[q.question.id] = orderingAnswers[q.question.id] || items.map(item => item.id);
      }
    });
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      e.preventDefault();
      e.returnValue = 'Та шалгалтаас гарахдаа итгэлтэй байна уу? Хадгалсан хариултууд хадгалагдана.';
  ...
  const renderQuestion = () => {
    ...
    switch (q.type) {
        ...
        return (
          <div className='space-y-3'>
            ...
            {q.options?.map(option => (
              <label
                key={option.id}
                className={`flex items-start gap-4 p-4 border-2 rounded-lg cursor-pointer transition-all ${answers[questionId] === option.id
        ...
        return (
          <div className='space-y-3'>
            {q.options?.map(option => (
              <label
                key={option.id}
                className={`flex items-start gap-4 p-4 border-2 rounded-lg cursor-pointer transition-all ${answers[questionId]?.includes(option.id)
                  ? 'border-blue-600 bg-blue-50 shadow-md scale-[1.02]'
                  : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50'
                  }`}>
                <input
        ...
        return (
          <div className='space-y-3'>
            {[true, false].map(value => (
              <label
                key={value.toString()}
                className={`flex items-center gap-4 p-4 border-2 rounded-lg cursor-pointer transition-all ${answers[questionId] === value
                  ? 'border-blue-600 bg-blue-50 shadow-md scale-[1.02]'
                  : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50'
                  }`}>
                <input
                ...
                />
                <span className='text-lg font-medium text-gray-800'>{value ? 'Үнэн' : 'Худал'}</span>
              </label>
            ))}
          </div>
        );
            ...
            {q.blanks?.map((blank, index) => (
              <div key={blank.id} className='space-y-2'>
                ...
                <input
                  ...
                  onChange={e => {
                    const newFillAnswer = { ...fillAnswer, [blank.id]: e.target.value };
      ...
      case 'matching':
        ...
        return (
          <div className='space-y-4'>
            ...
            {q.pairs?.map(pair => (
              <div key={pair.id} className='flex items-center gap-4 p-4 border-2 border-gray-200 rounded-lg bg-gray-50'>
                <div className='flex-1 font-medium text-gray-900'>{pair.left}</div>
                <div className='text-gray-400'>→</div>
                <select
                  value={matchingAnswer[pair.id] || ''}
                  onChange={e => {
                    const newMatchingAnswer = { ...matchingAnswer, [pair.id]: e.target.value };
      ...
      case 'ordering':
        ...
        const orderedItems = order.map(id => q.items?.find(item => item.id === id)).filter(Boolean);
        return (
          <div className='space-y-3'>
            <p className='text-sm text-gray-600 mb-4'>
            ...
            {orderedItems.map((item, index) => (
              <div
                ...
                <div className='flex flex-col gap-1'>
                  <button
                    type='button'
                    onClick={() => handleOrderingChange(questionId, item.id, 'up')}
                    disabled={index === 0}
                  ...
                  <button
                    type='button'
                    onClick={() => handleOrderingChange(questionId, item.id, 'down')}
        ...
        return (
          <div className='space-y-4'>
            <input
              ...
              placeholder={q.number_range ? `${q.number_range.min}-${q.number_range.max} хооронд тоо` : 'Тоон хариулт...'}
            />
            {q.number_range && (
              <p className='text-sm text-gray-600'>
                Утга: {q.number_range.min} - {q.number_range.max} хооронд байх ёстой
              </p>
            )}
            {answers[questionId] && (
              <p className='text-xs text-green-600 flex items-center gap-1'>
      ...
      default:
        return <p className='text-gray-600'>Энэ төрлийн асуулт дэмжигдэхгүй байна.</p>;
    }
  };
  ...
  const visibleStartIndex = questionsPerPage === 'all'
    ? 0
    : Math.floor(currentQuestionIndex / (parseInt(questionsPerPage) || 1)) * (parseInt(questionsPerPage) || 1);
    ...
    <div className='max-w-7xl mx-auto space-y-6 pb-6'>
            ...
            <span className='text-sm text-gray-600'>
              ...
              {lastSaveTime && (
                <span className='text-xs text-gray-500 ml-1'>
                  ({lastSaveTime.toLocaleTimeString('mn-MN', { hour: '2-digit', minute: '2-digit' })})
                </span>
              )}
            </span>
          </>
            ...
            {questions.map((q, index) => {
              ...
              return (
                <button
                  ...
                  className={`p-2 rounded-lg border-2 text-xs font-medium transition-all ${isCurrent
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              <p className='font-bold text-lg'>
                Анхааруулга: {timeRemaining < 60 ? '1 минут' : '5 минут'} үлдлээ!
              </p>
      ...
      <div className='bg-white rounded-lg shadow-lg p-6 sticky top-0 z-10 border-b-4 border-blue-600'>
        <div className='flex justify-between items-center mb-4'>
          ...
          <div className='flex items-center gap-4'>
            ...
            <div className={`flex items-center gap-3 px-5 py-3 rounded-lg font-bold text-lg transition-all ${isLowTime
        ...
        <div className='mb-2'>
          <div className='flex justify-between text-sm font-medium text-gray-700 mb-2'>
            ...
            <span className='flex items-center gap-4'>
              ...
              <span className='text-orange-600'>Хариулаагүй: {unansweredCount}</span>
      ...
      <div className='space-y-6'>
        {visibleQuestions.map((questionItem, visibleIndex) => {
          ...
          return (
            <div key={questionItem.id} className='bg-white rounded-lg shadow-lg p-8'>
              <div className='mb-6'>
                <div className='flex items-center gap-3 mb-4'>
                  ...
                  <span className='text-sm text-gray-600 bg-gray-100 px-3 py-1 rounded-full'>
                    {q.type === 'image_question' && <ImageIcon size={14} className='inline mr-1' />}
                    {q.type === 'matching' && '↔️ '}
                    {q.type === 'fill_blank' && '___ '}
                    {q.type === 'ordering' && <GripVertical size={14} className='inline mr-1' />}
      ...
      <div className='bg-white rounded-lg shadow-lg p-6'>
        <div className='flex justify-between items-center'>
          ...
          <div className='flex gap-3'>
            {(() => {
              const perPage = questionsPerPage === 'all' ? questions.length : parseInt(questionsPerPage) || 1;
              const currentPage = Math.floor(currentQuestionIndex / perPage);
                ...
                <button
                  onClick={() => {
                    if (questionsPerPage === 'all') {
                      ...
                      const nextIndex = Math.min(questions.length - 1, currentQuestionIndex + perPage);
        ...
        {questionsPerPage !== 'all' && (
          <div className='text-center text-sm text-gray-600 mt-4'>
            Хуудас {Math.floor(currentQuestionIndex / (parseInt(questionsPerPage) || 1)) + 1} / {Math.ceil(questions.length / (parseInt(questionsPerPage) || 1))}
          </div>
        )}
      </div>
          ...
          {questions.map((q, index) => {
            ...
            return (
              <button
                key={q.id}
                onClick={() => setCurrentQuestionIndex(index)}
                className={`p-2 rounded-lg border-2 text-xs font-medium transition-all ${isAnswered
        ...
        <div className='grid grid-cols-5 md:grid-cols-10 gap-2'>
          {questions.map((q, index) => (
            <button
              key={q.id}
              onClick={() => setCurrentQuestionIndex(index)}
              className={`p-3 rounded-lg border-2 text-sm font-medium transition-all ${index === currentQuestionIndex
                ? 'border-blue-600 bg-blue-600 text-white shadow-lg scale-110'
          ...
          <div className='bg-white rounded-lg shadow-2xl max-w-md w-full p-6 animate-in fade-in zoom-in'>
            <div className='mb-4'>
              ...
              <p className='text-gray-700 mb-4'>
                Та шалгалтаа илгээхдээ итгэлтэй байна уу? Илгээсний дараа засах боломжгүй.
              </p>
              <div className='bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4'>
                <p className='text-sm text-yellow-800'>
                  <strong>Хариулсан:</strong> {answeredCount} / {questions.length} асуулт
                </p>
                {unansweredCount > 0 && (
